...
function remove_existing () {
    if (imgExists) {
        meme.parentNode.removeChild(imgElement)
        imgExists = false
    }

    else if (jokeExists) {
        joke.parentNode.removeChild(jokeElement)
        jokeExists = false
    }

    else if (quoteExists) {
        quote.parentNode.removeChild(quoteElement)
        quoteExists = false
    }

    else if (riddleExists) {
        riddle.removeChild(riddleElement)
        riddleExists = false
    }
}
...
function show_meme (before, after, memes) {
    let img = document.createElement('img')
    src = memes[rn(memes.length)]
    img.src = src
    img.alt = 'image'
    before.parentNode.insertBefore(img, after)
}


const meme = document.querySelector('.meme-text');
const joke = document.querySelector('.joke-text');
const quote = document.querySelector('.quote-text');
const riddle = document.querySelector('.riddle-text');
...
const riddle_btn = document.querySelector('.riddle-btn');
const answer_btn = document.querySelector('.riddle-ans-btn');

let randomIndex
let imgExists = false
let imgElement
meme_btn.addEventListener('click', function () {
    let randomIndex = rn(memes.length);
    if (imgExists) {
        imgElement.src = memes[randomIndex]
    }
    else {
    remove_existing()
    ...
    meme.parentNode.insertBefore(imgElement, joke);
    }
    imgExists = true
});
...
joke_btn.addEventListener('click', function () {
    let randomIndex = rn(jokes.length)
    if (jokeExists) {
        ...
        jokeElement.textContent = jokes[randomIndex]
        joke.parentNode.insertBefore(jokeElement, quote)

        jokeElement.style.color = 'white'
        jokeElement.style.fontSize = '14px'
        jokeElement.style.marginTop = '10px'
        jokeElement.style.marginBottom = '10px'
    }
    jokeExists = true
})
...
quote_btn.addEventListener('click', function () {
    let randomIndex = rn(quotes.length)
    if (quoteExists) {
        quoteElement.innerHTML = quotes[randomIndex].quote + '<br>-' + quotes[randomIndex].author;
        ...
        quote.parentNode.insertBefore(quoteElement, riddle)
...
let riddleExists = false
let riddleElement
let currentIndex
let answerExists = false
riddle_btn.addEventListener('click', function () {
    let randomIndex = rn(riddles.length)
    currentIndex = randomIndex
    if (riddleExists) {
        riddleElement.innerHTML = riddles[randomIndex].question
    }
    else {
        remove_existing()
        ...
        riddle.appendChild(riddleElement)

        riddleElement.style.color = 'white'
        riddleElement.style.fontSize = '14px'
        riddleElement.style.marginTop = '10px'
        riddleElement.style.marginBottom = '10px'
    }
    riddleExists = true
    answerExists = false
})
...
answer_btn.addEventListener('click', function () {
    if (riddleExists && !answerExists) {
        riddleElement.innerHTML = riddleElement.innerHTML + '<br>' + riddles[currentIndex].answer
        answerExists = true
    }
});
